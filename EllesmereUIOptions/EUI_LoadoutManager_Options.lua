...
--
--  The module (EllesmereUILoadoutManager) owns the saved variables, the
--  context resolution and the swap machinery; this file is purely the surface.
--  Everything here goes through the ns API it publishes, so the page holds no
--  state of its own beyond which spec scope is being edited -- and even that
--  lives on the module, so a spec change while the page is open moves both.
-------------------------------------------------------------------------------
    ...
    local function BuildPage(pageName, parent, yOffset)
        ...
        _, h = W:DualRow(parent, y,
            ...
            { type = "toggle", text = "Swap Gear",
              ...
              setValue = function(v) ns.SetChannelEnabled("gear", v) end }
        ); y = y - h
        ...
        _, h = W:DualRow(parent, y,
            { type = "toggle", text = "Swap Talents",
              ...
              setValue = function(v) ns.SetChannelEnabled("talent", v) end },
        ...
        _, h = W:DualRow(parent, y,
            { type = "toggle", text = "Queue In Combat",
              ...
              setValue = function(v) ns.SetQueueInCombat(v) end },
        ...
        do
            ...
            _, h = W:DualRow(parent, y,
                { type = "dropdown", text = "Specialization Scope", values = values, order = order,
                  getValue = GetScopeKey, setValue = SetScopeKey,
                  tooltip = "Which layer the assignments below are saved to. All Specs applies to every specialization; a spec scope overrides it for that spec only." },
