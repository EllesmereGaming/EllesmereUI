    ...
    local function BuildPage(pageName, parent, yOffset)
        ...
        if not EllesmereUI._prebuilding then
            ...
            local kbBtn = EllesmereUI.BuildKeybindButton(rgn, {
                ...
                set = function(key)
                    -- Unbound is stored as false, the profile default
                    Set("toggleKey", key or false)
                    Refresh()
        ...
        if not EllesmereUI._prebuilding then
            local function AddQuickFireKeybind(region, key)
                local button = EllesmereUI.BuildKeybindButton(region, {
                    mouse = true,
                    disabled = QuickFireDisabled,
                    disabledReason = "Enable Quick Fire",
                    get = function() return Cfg(key) end,
                    set = function(chord)
                        Set(key, chord or false)
                        Refresh()
                ...
                button.Refresh()
                EllesmereUI.RegisterWidgetRefresh(button.Refresh)
