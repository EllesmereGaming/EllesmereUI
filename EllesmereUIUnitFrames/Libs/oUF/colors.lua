...
local colorMeta = {
	__index = function(self, key)
		...
		elseif(key == 'a') then
			return rawget(self, 4) or 1
		end

		return colorMethods[key]
	end,
...
function colorMethods:GetRGB()
	return self[1], self[2], self[3]
end

function colorMethods:GetRGBA()
	return self[1], self[2], self[3], self[4] or 1
end

function colorMethods:SetRGB(r, g, b)
	self[1], self[2], self[3] = r, g, b
end

function colorMethods:SetRGBA(r, g, b, a)
	self[1], self[2], self[3], self[4] = r, g, b, a
end

function colorMethods:GetRGBAsBytes()
	return self[1] * 255, self[2] * 255, self[3] * 255
end

function colorMethods:GenerateHexColor()
	return string.format('ff%02x%02x%02x', self[1] * 255, self[2] * 255, self[3] * 255)
end

function colorMethods:GenerateHexColorMarkup()
...
function colorMethods:IsEqualTo(other)
	return other and self[1] == other[1] and self[2] == other[2]
		and self[3] == other[3] and (self[4] or 1) == (other[4] or 1)
end

function colorMethods:SetAtlas(atlas)
	self.atlas = atlas
end

function colorMethods:GetAtlas()
	return self.atlas
end

function colorMethods:SetCurve(curve)
	self.curve = curve
end

function colorMethods:GetCurve()
	return self.curve
end
...
local function CreateColor(r, g, b, a)
	...
	return setmetatable({r, g, b, a}, colorMeta)
end
	...
	if(CUSTOM_CLASS_COLORS) then
		local function updateColors()
			for classToken, color in next, CUSTOM_CLASS_COLORS do
				colors.class[classToken] = CreateColor(color.r, color.g, color.b)
			end
...
local function upgradeColorTables(tbl)
	for _, value in next, tbl do
		if(type(value) == 'table') then
			if(type(value[1]) == 'number' and type(value[2]) == 'number'
				and type(value[3]) == 'number' and #value <= 4) then
				setmetatable(value, colorMeta)
			else
				upgradeColorTables(value)
			end
		end
	end
end

upgradeColorTables(colors)

-- Current oUF calls this collection "dispel"; keep "debuff" as the legacy
-- alias used by Wrath layouts and the aura element.
colors.dispel = colors.debuff
