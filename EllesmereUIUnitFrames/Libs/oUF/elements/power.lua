...
local function UpdateColor(self, event, unit)
	...
	local color = t or (b and oUF:CreateColor(r, g, b))
	...
	if(element.PostUpdateColor) then
		element:PostUpdateColor(unit, color, altR, altG, altB)
	end
end
...
local function SetColorTapping(element, state)
	if(element.colorTapping ~= state) then
		...
		elseif(not element.colorSelection and not element.colorReaction) then
...
local function SetColorSelection(element, state)
	if(element.colorSelection ~= state) then
		...
		if(state) then
			element.__owner:RegisterEvent('UNIT_FACTION', ColorPath)
		elseif(not element.colorTapping and not element.colorReaction) then
			element.__owner:UnregisterEvent('UNIT_FACTION', ColorPath)
		end
	end
end

local function SetColorReaction(element, state)
	if(element.colorReaction ~= state) then
		element.colorReaction = state
		if(state) then
			element.__owner:RegisterEvent('UNIT_FACTION', ColorPath)
		elseif(not element.colorTapping and not element.colorSelection) then
			element.__owner:UnregisterEvent('UNIT_FACTION', ColorPath)
		end
	end
end
...
local function Enable(self, unit)
	...
	if(element) then
		...
		if(element.colorTapping or element.colorSelection or element.colorReaction) then
