...
frame:SetScript('OnEvent', function(self, event, unit)
	...
	if(strings) then
		for _, fontstring in next, strings do
			if(fontstring:IsVisible() and (unitlessEvents[event] or fontstring.parent.unit == unit
				or (fontstring.extraUnits and fontstring.extraUnits[unit]))) then
...
local function getTagData(tag)
	local inner = tag:sub(2, -2)
	local customArgs = inner:match('%((.-)%)$')
	if(customArgs) then
		inner = inner:gsub('%(.-%)$', '')
	end

	-- Current syntax uses explicit $ markers, which avoids ambiguity when a
	-- prefix or suffix itself contains angle brackets.
	if(inner:find('$>', 1, true) or inner:find('<$', 1, true)) then
		local prefix, rest = inner:match('^(.-)%$>(.*)$')
		if(not rest) then rest = inner end

		local name, suffix = rest:match('^(.-)<%$(.*)$')
		return name or rest, prefix, suffix, customArgs
	end

	-- Preserve the original Wrath syntax: [prefix>tag<suffix].
	local name, tagStart, tagEnd = getTagName(tag)
	local bareName, legacyArgs = name:match('^(.-)%((.-)%)$')
	name = bareName or name
	customArgs = customArgs or legacyArgs

	tagStart = tagStart - 2
	tagEnd = tagEnd + 2
	local prefix = tagStart ~= 0 and tag:sub(2, tagStart)
	local suffix = tagEnd ~= 0 and tag:sub(tagEnd, -2)
	return name, prefix, suffix, customArgs
end
...
local function registerEvents(fontstr, tagstr)
	for bracket in tagstr:gmatch(_PATTERN) do
		local tagName = getTagData(bracket)
		local tagevents = tagEvents[tagName]
		if(tagevents) then
...
local function Tag(self, fs, tagstr, ...)
	...
	fs.parent = self
	if(...) then
		fs.extraUnits = fs.extraUnits or {}
	...
	for bracket in tagstr:gmatch(_PATTERN) do
		local tagName = getTagData(bracket)
		if not tagEvents[tagName] then
			containsOnUpdate = onUpdateDelay[tagName] or 0.15;
		end
	end
	...
	if(not func) then
		...
		for bracket in tagstr:gmatch(_PATTERN) do
			...
			if(not tagFunc) then
				local tagName, prefix, suffix, customArgs = getTagData(bracket)

				local tag = tags[tagName]
				if(tag) then
					local function callTag(unit, realUnit)
						if(customArgs) then
							return tag(unit, realUnit, string.split(',', customArgs))
						end
						return tag(unit, realUnit)
					end

					if(prefix and suffix) then
						tagFunc = function(unit, realUnit)
							local str = callTag(unit, realUnit)
							if(str) then
...
oUF:RegisterMetaFunction('UpdateTags', onShow)
