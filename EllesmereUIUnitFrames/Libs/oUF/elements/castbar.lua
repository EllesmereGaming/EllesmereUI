...
local function CastStart(self, event, unit)
	...
	if(not (element.ShouldShow or ShouldShow)(element, unit)) then return end

	-- A unit swap or a missing stop event must not leak state into the new cast.
...
local function CastFail(self, event, unit, _, _, castID)
	...
	* unit    - the unit for which the update has been triggered (string)
	--]]
	if(event == 'UNIT_SPELLCAST_INTERRUPTED' and element.PostCastInterrupted) then
		return element:PostCastInterrupted(unit)
	elseif(element.PostCastFail) then
