...
local function onEvent(self, event, ...)
	if(not self:IsVisible() and event ~= 'UNIT_COMBO_POINTS') then return end

	local unit = ...
	local unitless = self.__unitlessEvents and self.__unitlessEvents[event]
	if(not unitless and event:match('^UNIT_') and unit ~= self.unit and unit ~= self.realUnit) then
		return
	end
...
function frame_metatable.__index:RegisterEvent(event, func, unitless)
	...
	argcheck(unitless, 4, 'boolean', 'nil')

	-- Events without a UNIT_ prefix never carry a unit token. The explicit
	-- argument is still required for exceptional UNIT_ events such as
	-- UNIT_COMBO_POINTS on the 3.3.5 client.
	if(unitless or not event:match('^UNIT_') or self.__eventless) then
		self.__unitlessEvents = self.__unitlessEvents or {}
		self.__unitlessEvents[event] = true
	end
...
function frame_metatable.__index:UnregisterEvent(event, func)
	...
	if(cleanUp or curev == func) then
		self[event] = nil
		if(self.__unitlessEvents) then
			self.__unitlessEvents[event] = nil
		end
