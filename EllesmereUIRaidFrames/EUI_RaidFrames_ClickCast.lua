...
-- Rez). Returns nil when there is neither a natural rez kit nor a configured
-- combat-rez item. Never includes /stopmacro -- caller adds that for oocOnly.
...
local function BuildRezLines(binding, guard, standalone)
    ...
    if not kit and not rezItem then return nil end
    local bank = Enum.SpellBookSpellBank and Enum.SpellBookSpellBank.Player
