    ...
    local function ShowTBBSpellPicker(anchorFrame, barCfg, onChanged)
        ...
        local function MakeSpellItem(sp)
            ...
            if usedOnBar and not isSelected then
                ...
                item:SetScript("OnEnter", function()
                    EllesmereUI.ShowWidgetTooltip(item, EllesmereUI.Lf("Already assigned to %1$s", usedOnBar))
    ...
    local function ShowSpellPicker(anchorFrame, barKey, slotIndex, excludeSet, onSelect, removeOnly)
        ...
        if removeOnly then
            ...
            if slotIndex and not isCustomBuff then
                ...
                if spellID and spellID ~= 0 then
                    ...
                    local function MakeSubnavRow(label, items, getVal, setVal, isDefault, onItemCreated, opts)
                        ...
                        local function ShowSub()
                            ...
                            for _, item in ipairs(items) do
                                ...
                                flyoutEntries[#flyoutEntries + 1] = { frame = si, label = sLbl, name = item.label }
                                subH = subH + ITEM_H
                    ...
                    local procRow = MakeSubnavRow("Proc Glow", GLOW_ITEMS,
                        ...
                        function(si, item)
                            ...
                            if isGlow and (cse == "pixelGlowReady" or cse == "buttonGlowReady") then
                                ...
                                si:SetScript("OnEnter", function()
                                    EllesmereUI.ShowWidgetTooltip(si, EllesmereUI.L("Disable CD Ready glow first"))
                                end)
                    ...
                    -- per-icon sync, so the whole toggle is skipped for them. Preset
                    -- icons (custom injected) have no per-spell sync either -- their
                    -- only setting is the custom Active State.
                    local isCustomInjectedSync = spellID < 0
                        or (ns._myRacialsSet and ns._myRacialsSet[spellID])
                        or (sd.customSpellIDs and sd.customSpellIDs[spellID])
                    if not isBuffBar and not isCustomInjectedSync then
            ...
            for _, slot in ipairs({13, 14}) do
                ...
                if isDisabled then
                    ...
                    ti:SetScript("OnEnter", function()
                        EllesmereUI.ShowWidgetTooltip(ti, EllesmereUI.Lf("Already on %1$s", tooltipName))
                    end)
            ...
            if rSid then
                ...
                if rIsDisabled then
                    ...
                    ri:SetScript("OnEnter", function()
                        EllesmereUI.ShowWidgetTooltip(ri, EllesmereUI.Lf("Already on %1$s", rTooltipName))
                    end)
        ...
        local function MakeItem(sp, isDisabled)
            ...
            elseif onThisBar then
                ...
                item:SetScript("OnEnter", function()
                    EllesmereUI.ShowWidgetTooltip(item, EllesmereUI.Lf("This spell is already being used on %1$s", barName))
                    hl:SetColorTexture(1, 1, 1, hlA * 0.3); hl:SetAlpha(1)
                end)
                item:SetScript("OnLeave", function()
                    EllesmereUI.HideWidgetTooltip()
        ...
        if isFocusKick then
            ...
            do
                ...
                accentSwatch:SetScript("OnEnter", function()
                    EllesmereUI.ShowWidgetTooltip(accentSwatch, EllesmereUI.L("Accent Color"))
                end)
