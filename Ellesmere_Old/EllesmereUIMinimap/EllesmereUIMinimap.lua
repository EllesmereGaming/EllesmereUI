...
local function EnsureFlyoutPanel()
    if not flyoutPanel then
        flyoutPanel = CreateFrame("Frame", nil, UIParent)
        ...
        for idx, entry in ipairs(ungrouped) do
            ...
            if showBg then
                ...
                if not GetFFD(btn).ungroupBg then
                    local ubg = CreateFrame("Frame", nil, btn)
...
local function ApplyMinimap()
    ...
    -- Shape mask
    local maskPath = isCircle and "Interface\\CharacterFrame\\TempPortraitAlphaMask" or "Interface\\Buttons\\WHITE8X8"
    ...
    if clockMode ~= "none" then
        if not clockBg then
            clockBg = CreateFrame("Button", nil, minimap)
    ...
    if locationMode ~= "none" then
        if not locationBg then
            locationBg = CreateFrame("Frame", nil, minimap)
...
do
    ...
    local function CreateMenuFrame()
        menuFrame = CreateFrame("Frame", "EllesmereUIMicroMenu", UIParent)
