...
local function GetOrCreateSlot(idx)
    ...
    local btn = CreateFrame("Button", nil, slotParent, "ContainerFrameItemButtonTemplate")
...
local function RenderButton(btn, data, _, col, row, startX, currentY, _, interactiveEmpties)
    ...
    if not data.info then
        ...
        SetItemButtonTexture(btn, data.info.iconFileID)
        SetItemButtonCount(btn, data._mergedCount or data.info.stackCount)
...
function EUI_BagsReagent:RefreshInventory()
    ...
    for i, data in ipairs(tempItems) do
        ...
        if not data.info then
            ...
            if btn.icon then btn.icon:Show() end
            SetItemButtonTexture(btn, data.info.iconFileID)
            SetItemButtonCount(btn, data.info.stackCount)
