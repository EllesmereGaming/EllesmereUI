...
local function ProcessSpecChange(newSpecKey)
    ...
    -- options' own ACTIVE_TALENT_GROUP_CHANGED watcher races this swap (that
...
-- ZONE_CHANGED_NEW_AREA, and ACTIVE_TALENT_GROUP_CHANGED.
...
local function EnsureFocusReminderProxy()
    ...
    _focusReminderProxy:RegisterEvent("ACTIVE_TALENT_GROUP_CHANGED")
...
-- if multiple wakeup events fire (PLAYER_LOGIN + first ACTIVE_TALENT_GROUP_CHANGED).
...
function ECME:CDMFinishSetup()
    ...
    if not _cdmVehicleProxy then
        ...
        _cdmVehicleProxy:SetAttribute("_onstate-cdmvehicle", [[
            control:CallMethod("OnVehicleStateChanged", newstate)
        ]])
...
eventFrame:SetScript("OnEvent", function(_, event, unit, updateInfo, arg3)
    ...
    if event == "GLYPH_UPDATED" or event == "PLAYER_TALENT_UPDATE" or event == "ACTIVE_TALENT_GROUP_CHANGED"
    ...
    if event == "ACTIVE_TALENT_GROUP_CHANGED" and unit == "player" then
