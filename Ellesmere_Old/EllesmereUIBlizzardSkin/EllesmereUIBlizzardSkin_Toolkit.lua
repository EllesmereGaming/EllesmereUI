...
function WSkin:HandleCheckBox(frame, noBackdrop, noReplaceTextures)
	...
	if not noReplaceTextures then
		if frame.SetCheckedTexture then
			...
			if noBackdrop then frame:GetCheckedTexture():SetInside(nil, -4, -4) end
		end
		if frame.SetDisabledCheckedTexture then
			frame:SetDisabledCheckedTexture("Interface\\Buttons\\UI-CheckBox-Check-Disabled")
			if noBackdrop then frame:GetDisabledCheckedTexture():SetInside(nil, -4, -4) end
		end
		if frame.SetDisabledTexture then
			frame:SetDisabledTexture("Interface\\Buttons\\UI-CheckBox-Check-Disabled")
			if noBackdrop then frame:GetDisabledTexture():SetInside(nil, -4, -4) end
		end
...
function WSkin:HandleTab(tab, noBackdrop)
	...
	if highlightTex then highlightTex:SetTexture() else tab:StripTextures() end
...
function WSkin:HandleNextPrevButton(btn, arrowDir, color, noBackdrop, stipTexts)
	...
	if Normal then Normal:SetInside() end
	if Pushed then Pushed:SetInside() end
	if Disabled then Disabled:SetInside() end
