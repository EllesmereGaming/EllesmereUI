...
local function PreSkinCharacterSheet()
    ...
    for _, f in ipairs({_G.CharacterMainHandSlot, _G.CharacterSecondaryHandSlot}) do
        local r16, r17 = select(16, f:GetRegions())
...
local function SkinCharacterSheet()
    ...
    _ComputeBetterInventoryItems = function()
        ...
        for bagSlot = 0, 5 do
            local bagSize = (C_Container and C_Container.GetContainerNumSlots or GetContainerNumSlots)(bagSlot)
            for slotIndex = 1, bagSize do
                local itemLink = (C_Container and C_Container.GetContainerItemLink or GetContainerItemLink)(bagSlot, slotIndex)
                if itemLink then
    ...
    iLvlUpdateFrame:RegisterEvent("ACTIVE_TALENT_GROUP_CHANGED")
    ...
    local function GetStatSectionsOrder()
        -- TODO: Need to fix so that we display WOTLK only ratings and not Legion+ 
        -- EllesmereUIBlizzardSkin_CharacterSheet needs to be modified.
        local defaultOrder = {
            {
                title = "Attributes",
                colorKey = "Attributes",
    ...
    local function ApplyCustomSlotBorder(slotName)
        ...
        -- Crop icon inward (3.3.5 WotLK uses _G[slotName .. "IconTexture"])
        ...
        -- Hide NormalTexture
        local normalTexture = _G[slotName .. "NormalTexture"] or (slot.GetNormalTexture and slot:GetNormalTexture())
        if normalTexture then
        ...
        if EllesmereUI and EllesmereUI.PanelPP then
            ...
            if bdrFrame then bdrFrame:SetFrameLevel(slot:GetFrameLevel() + 5) end
        end
    end
