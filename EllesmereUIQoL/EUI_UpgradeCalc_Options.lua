...
-- Fallback: ITEM_UPGRADE_MASTER_UPDATE fires when the NPC sends upgrade data.
--           Guarded with ItemUpgradeFrame:IsShown() so it only triggers when
--           the upgrade panel is actually visible.
local _upgraderRegistered = false
local function RegisterUpgraderHook()
    if _upgraderRegistered then return end
    _upgraderRegistered = true
    local upgraderEvtFrm = CreateFrame("Frame")
    upgraderEvtFrm:RegisterEvent("PLAYER_INTERACTION_MANAGER_FRAME_SHOW")
    upgraderEvtFrm:RegisterEvent("PLAYER_INTERACTION_MANAGER_FRAME_HIDE")
    upgraderEvtFrm:RegisterEvent("ITEM_UPGRADE_MASTER_UPDATE")
    upgraderEvtFrm:SetScript("OnEvent", function(self, event, interactionType)
        local upgradeType = Enum.PlayerInteractionType
                         and Enum.PlayerInteractionType.ItemUpgrade

        if event == "PLAYER_INTERACTION_MANAGER_FRAME_SHOW" then
            if not upgradeType then return end
            if interactionType ~= upgradeType then return end
            if not GetAddonDB().openWithUpgrader then return end
            local fr = _G["EUIUpgCalcFrame"]
        ...
        -- ITEM_UPGRADE_MASTER_UPDATE fallback (open only)
        if not (ItemUpgradeFrame and ItemUpgradeFrame:IsShown()) then return end
        if not GetAddonDB().openWithUpgrader then return end
        local fr = _G["EUIUpgCalcFrame"]
        if fr and not fr:IsShown() then fr:Show() end
    end)
