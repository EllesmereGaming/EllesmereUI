...
--  window; that cog is shown only to a raid leader or assistant, since nobody
...
--
--  NOT SECURE, AND THAT IS NOT AN OVERSIGHT. SetRaidSubgroup and
--  SwapRaidSubgroup are ordinary functions -- no lockdown restriction, no
--  taint path -- so unlike the Raid Tools panels this window needs no secure
--  frames, no state driver and no combat deferral. Plain frames throughout.
--
...
-- On-screen text resolves its font through the QoL ("extras") key, so it
-- follows the user's Global Font, outline and shadow settings like every other
-- QoL window. EllesmereUI.MakeFont is the options-panel helper and hardcodes
-- that panel's font, which would leave the player names -- the whole point of
-- this window -- as the one text in QoL that ignores the setting. Resolved at
-- build, like the other QoL popups: a font change made after the first open
-- shows after a reload.
local function MakeText(parent, size, r, g, b)
    local fs = parent:CreateFontString(nil, "OVERLAY")
    local flags = EllesmereUI.GetFontOutlineFlag("extras")
    EllesmereUI.PrimeFontShadow(fs, flags == "")
    fs:SetFont(EllesmereUI.GetFontPath("extras"), size, flags)
...
local dropTarget                -- the slot currently wearing the accent outline
local targetBrd                 -- the ONE outline, moved onto dropTarget (see Build)
...
local Refresh   -- forward: the drop handler and the roster handler both call it
...
--   outline accent line -- "release now and it lands HERE". ONE frame, moved
--                         onto the target: MakeBorder registers a permanent
--                         scale callback per call, so a border per slot would
--                         leave forty of them behind to light one at a time
...
local function SetSlotTarget(s, on)
    SetSlotHover(s, on)
    local outline = targetBrd._frame
    if on then
        local ar, ag, ab = EllesmereUI.GetAccentColor()
        outline:SetAllPoints(s)
        targetBrd:SetColor(ar, ag, ab, 1)
        outline:Show()
    else
        outline:Hide()
    end
...
--
-- Slots are asked directly with IsMouseOver.
--
...
local function GhostOnUpdate(self)
    ...
    -- The cursor stays on one slot for most frames, so the current target is
    -- asked first; the scan over the grid is only for when it has moved.
    local over
    if dropTarget and dropTarget:IsMouseOver() then
        over = dropTarget
    elseif win:IsMouseOver() then
        for i = 1, #slots do
...
local function DropDrag()
    ...
    StopDrag()
    if not target or not ns.RaidGroupsPermitted() then return end
...
local function MakeSlot(parent, group)
    ...
    s._lbl = MakeText(s, NAME_SIZE, 1, 1, 1)
    ...
    -- per slot would allocate 160 identical closures at first open.
...
-- Runs only while the window is up (see Build): the listener is registered on
-- show and dropped on hide, so a session that never opens this window never
-- hears GROUP_ROSTER_UPDATE on its behalf, and one that does hears it only for
-- as long as there is something on screen to repaint.
...
local function Build()
    ...
    -- Created here, on first open, rather than at load. HookScript because
    -- nothing else on this frame should be displaced by it; Escape and the
    -- close button both go through Hide, so OnHide is the one place it ends.
    local roster = CreateFrame("Frame")
    roster:SetScript("OnEvent", OnRosterUpdate)
    win:HookScript("OnShow", function() roster:RegisterEvent("GROUP_ROSTER_UPDATE") end)
    win:HookScript("OnHide", function()
        -- Closed mid-drag (Escape, the close button): the drag ends here rather
        -- than waiting on OnDragStop to reach a slot that has just been hidden,
        -- so the ghost and its per-frame OnUpdate cannot outlive the window.
        if dragSlot then StopDrag() end
        roster:UnregisterEvent("GROUP_ROSTER_UPDATE")
    end)

    -- The panel scale slider notifies through this shared list; ours folds the
    -- per-window multiplier back in. Registered here, on first open, so a session
    -- that never opens the window has no entry in it.
    if not EllesmereUI._onScaleChanged then EllesmereUI._onScaleChanged = {} end
    EllesmereUI._onScaleChanged[#EllesmereUI._onScaleChanged + 1] = ApplyScale
    ...
    ghost._lbl = MakeText(ghost, NAME_SIZE, 1, 1, 1)
    ...
    local title = MakeText(win, 13, EllesmereUI.GetAccentColor())
    ...
    local closeLbl = MakeText(close, 14, 1, 1, 1)
    ...
    for g = 1, NUM_GROUPS do
        ...
        local hdr = MakeText(win, HEADER_SIZE, 1, 1, 1)
        hdr:SetAlpha(0.55)
    ...
    -- The one drop-target outline: created after the slots and raised above
    -- them, then moved onto whichever slot is the target (SetSlotTarget).
    targetBrd = EllesmereUI.MakeBorder(win, 1, 1, 1, 0, EllesmereUI.PP)
    targetBrd._frame:SetFrameLevel(win:GetFrameLevel() + 3)
    targetBrd._frame:Hide()
end
...
-- One-shot login init, the same shape RaidTools.lua and every other QoL feature
-- use: take the shared DB handle and be done. Nothing else in this file creates
-- a frame, registers an event or adds a callback until the window is first
-- opened (see Build), and the cog that opens it exists only once Enable Raid
-- Groups Window is on.
